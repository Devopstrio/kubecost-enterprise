...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Cost Intelligence', path: '/', icon: LayoutDashboard },
    { name: 'Allocation Engine', path: '/allocation', icon: PieChart },
    { name: 'Workload Analysis', path: '/workloads', icon: Layers },
    { name: 'Predictive Forecast', path: '/forecast', icon: TrendingUp },
    { name: 'Optimization Hub', path: '/optimization', icon: Zap },
    { name: 'Budget Governance', path: '/budget', icon: Target },
    { name: 'Chargeback Portal', path: '/chargeback', icon: DollarSign },
    { name: 'Unit Economics', path: '/economics', icon: ArrowUpRight },
    { name: 'Multi-Cloud Sync', path: '/cloud', icon: Cloud },
    { name: 'FinOps Settings', path: '/settings', icon: Settings },
  ];
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-emerald-400 bg-clip-text text-transparent">KubeCost.IQ</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 truncate">Cloud Economics</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search namespaces, clusters, cloud billing accounts..." 
